...
splitbyIndices str (x:xs) size =
    str !!! (0, x) : splitbyIndices (str !!! (x + size, length str)) xs size
splitbyIndices _ [] _ = []
...
split sub str =
    splitbyIndices
        str
        (normalizeArray (findString sub str) $ length sub)
        (length sub)
...
sanitizeMarkdown txt = do
    let newTxt =
            if "\'---\'" `isInfixOf` txt
                then T.unpack
                         (T.replace "\'---\'" "<< +THREEDASHES+ >>" (T.pack txt))
                else txt
    if newTxt !!! (length newTxt - 3, length newTxt) /= "---"
        then do
            newTxt ++ "---"
        else newTxt

removeExtensionMD :: String -> String
removeExtensionMD file =
    if ".md" `isInfixOf` file
        then file !!! (0, length file - 3)
        else file

getArgsIndex :: Int -> [String] -> Maybe String
getArgsIndex index args =
