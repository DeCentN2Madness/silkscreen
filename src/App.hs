...
generateFullPage title content template colors = do
    let sectionsList =
            replaceThreeDashes $
            map T.pack $ split "---" (sanitizeMarkdown content)
    let sections = map (commonmarkToHtml []) sectionsList
    let templateTitle =
            T.replace "<< +TITLE+ >>" (T.pack title) $ T.pack template
    let templateBackgroundColor =
            T.replace
                "<< +BACKGROUND COLOR+ >>"
                (generateBackgroundCSS (take (length sections) (cycle colors)))
                templateTitle
    let templateActions =
            T.replace
                "<< +ACTIONS+ >>"
                (generateActionsCSS (length sections))
                templateBackgroundColor
    let templateRadioButtons =
            T.replace
                "<< +RADIOBUTTONS+ >>"
                (generateRadioButtons (length sections))
                templateActions
    T.replace
        "<< +SECTIONS+ >>"
        (generateSectionsHTML (map T.unpack sections))
        templateRadioButtons

generateBackgroundCSS :: [String] -> T.Text
generateBackgroundCSS colors =
    T.pack $
    intercalate
        ""
        [ unlines
...
generateRadioButtons size =
    T.pack $
    intercalate
        ""
        [ if x == 1
            then "\t<input type=\"radio\" name=\"selectPage\" id=\"rd1\" checked=\"true\" autofocus>\n"
            else "\t<input type=\"radio\" name=\"selectPage\" id=\"rd" ++
                 show x ++ "\">\n"
        | x <- [1 .. size]
        ]
...
replaceThreeDashes sections =
    [ if "<< +THREEDASHES+ >>" `isInfixOf` T.unpack txt
        then T.replace "<< +THREEDASHES+ >>" "\'---\'" txt
        else txt
    | txt <- sections
    ]
